...
read_plus <- function(flnm) {
  read_csv(flnm) %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_type = ifelse(grep("dedza",flnm >= 1) | grep("mangochi",flnm >= 1) | grep("nkhatabay",flnm >= 1),"District","Tertiary"))
}
...
#Getting Error here too
 df_test = for(i in 1:nrow(patient_address_df)){
    mutate(patient_address_df[i,], health_facility_type = ifelse((length("dedza") >= 1),"District","Tertiary"))  
} 
