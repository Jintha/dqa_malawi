...
all_weight_encounters = all_health_facility_vitals %>% filter(encounter_datetime >= "2017-01-01") 

#%>% filter(weight != "NULL") %>% group_by(unique_patient_id,health_facility_name) %>% slice(1) 
...
#Fix the logic for checking calling of this loop here.
#I want to loop only when there is no cleaned file of encounters
...
pat_weight_vector = study_period_vitals[i]$weight
pat_height_vector = study_period_vitals[i]$height
...
for(i in 1:nrow(study_period_vitals)){
  ...
  plausible_vitals_weight[i] = ifelse(pat_weight_vector[i] >= validating_record$low_weight & pat_weight_vector[i] <= validating_record$high_weight,"Plausible","Not Plausible" )
  if(pat_height_vector[i] !is.null){
    plausible_vitals_height[i] = ifelse(pat_height_vector[i] >= validating_record$low_height & pat_height_vector[i] <= validating_record$high_height,"Plausible","Not Plausible" )
  }
}
study_period_vitals$plausible_weight  = plausible_vitals_weight
study_period_vitals$plausible_height = plausible_height
...
#Plausible Adherence
#
all_health_facility_adherence = as.numeric(all_health_facility_adherence$adherence_in_percentage)
...
number_of_patients_with_plausible_adherence = all_health_facility_adherence %>% filter(start_date_new >= "2017-01-01") %>%  filter(plausible_adherence == "Plausible") 
