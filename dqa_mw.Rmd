...
patient_weight_district_mangochi_df = fread("~/Box Sync/quantitative_data/patient_weight_height/District/mangochi_vitals.csv") %>% mutate(health_facility_type = "District") %>% mutate(health_facility_name = "Mangochi") %>% mutate(unique_patient_id = paste("mh", patient_id, sep="_"))

patient_weight_district_nkhatabay_df = fread("~/Box Sync/quantitative_data/patient_weight_height/District/nkhatabay_vitals.csv") %>% mutate(health_facility_type = "District") %>% mutate(health_facility_name = "Nkhatabay") %>% mutate(unique_patient_id = paste("nb", patient_id, sep="_"))

patient_weight_district_dedza_df = fread("~/Box Sync/quantitative_data/patient_weight_height/District/dedza_vitals.csv") %>% mutate(health_facility_type = "District") %>% mutate(health_facility_name = "Dedza") %>% mutate(unique_patient_id = paste("dz", patient_id, sep="_"))


patient_weight_tertiary_qech_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Tertiary/qech_vitals.csv") %>% mutate(health_facility_type = "Tertiary") %>% mutate(health_facility_name = "QECH") %>% mutate(unique_patient_id = paste("qech", patient_id, sep="_"))


patient_weight_tertiary_kch_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Tertiary/LH_vitals.csv")%>% mutate(health_facility_type = "Tertiary") %>% mutate(health_facility_name = "KCH") %>% mutate(unique_patient_id = paste("kch", patient_id, sep="_"))

patient_weight_tertiary_mch_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Tertiary/mzuzu_central.csv") %>% mutate(health_facility_type = "Tertiary") %>% mutate(health_facility_name = "MCH") %>% mutate(unique_patient_id = paste("mch", patient_id, sep="_"))

patient_weight_tertiary_zch_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Tertiary/zch_vitals.csv") %>% mutate(health_facility_type = "Tertiary") %>% mutate(health_facility_name = "ZCH") %>% mutate(unique_patient_id = paste("zch", patient_id, sep="_"))


patient_weight_healthcenter_limbe_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Health Center/limbe_vitals.csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Limbe") %>% mutate(unique_patient_id = paste("lim", patient_id, sep="_"))

patient_weight_healthcenter_kawale_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Health Center/kawale_vitals.csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Kawale") %>% mutate(unique_patient_id = paste("kaw", patient_id, sep="_"))

patient_weight_healthcenter_mzuzu_df = fread("~/Box Sync/quantitative_data/patient_weight_height/Health Center/mzuzu_mapale_vitals.csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Mapale") %>% mutate(unique_patient_id = paste("map", patient_id, sep="_"))
...
patient_drugs_healthcenter_limbe_df = fread("~/Box Sync/quantitative_data/patient_drugs_taken/Health Center/Limbe_query_drugs2.csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Limbe") %>% mutate(unique_patient_id = paste("lim", patient_id, sep="_"))

patient_drugs_healthcenter_kawale_df = fread("~/Box Sync/quantitative_data/patient_drugs_taken/Health Center/kawale_query_drugs2.csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Kawale") %>% mutate(unique_patient_id = paste("kaw", patient_id, sep="_"))

patient_drugs_healthcenter_mzuzu_df = fread("~/Box Sync/quantitative_data/patient_drugs_taken/Health Center/mzuzu_mapale_query_drugs2..csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Mapale") %>% mutate(unique_patient_id = paste("map", patient_id, sep="_"))
...
#patient_drug_adherence_files_district_mangochi = list.files(path="~/Box Sync/quantitative_data/patient_drug_adherence/District/", pattern="*.csv", full.names=TRUE)
#patient_drug_adherence_files_district_dedza = list.files(path="~/Box Sync/quantitative_data/patient_drug_adherence/District/", pattern="*.csv", full.names=TRUE)
...
#patient_drug_adherence_files_tertiary_kch = list.files(path="~/Box Sync/quantitative_data/patient_drug_adherence/Tertiary/", pattern="*.csv", full.names=TRUE)
...
patient_drug_adherence_healthcenter_kawale_df = fread("~/Box Sync/quantitative_data/patient_drug_adherence/Health Center/kawale_query_adherence2.csv") %>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Kawale") %>% mutate(unique_patient_id = paste("kaw", patient_id, sep ="_"))
#patient_drug_adherence_files_healthcenter_limbe = list.files(path="~/Box Sync/quantitative_data/patient_drug_adherence/Health Center/", pattern="*.csv", full.names=TRUE)
patient_drug_adherence_healthcenter_mzuzu_df = fread("~/Box Sync/quantitative_data/patient_drug_adherence/Health Center/mzuzu_mapale_HC_adherence.csv")%>% mutate(health_facility_type = "Health Center") %>% mutate(health_facility_name = "Mapale") %>% mutate(unique_patient_id = paste("map", person_id, sep="_"))
...
patient_weight_list = list(patient_weight_healthcenter_mzuzu_df,patient_weight_healthcenter_kawale_df,patient_weight_healthcenter_limbe_df,patient_weight_tertiary_mch_df,patient_weight_tertiary_qech_df,patient_weight_tertiary_zch_df,patient_weight_tertiary_kch_df,patient_weight_district_dedza_df,patient_weight_district_mangochi_df,patient_weight_district_nkhatabay_df)

patient_adherence_list = list(patient_drug_adherence_district_nkhatabay_df,patient_drug_adherence_healthcenter_kawale_df,patient_drug_adherence_healthcenter_mzuzu_df,patient_drug_adherence_files_tertiary_qech,patient_drug_adherence_tertiary_mch_df,patient_drug_adherence_tertiary_zch_df) # healthcenter data frames are off
...
d = table1(~gender + age|health_facility_name, data = all_health_facility_addresses, output = "pandoc")
...
all_prescription_plausibility_proportion = number_of_all_plausible_prescriptions / number_of_all_complete_prescriptions  *100
...
foreach (1:(nrow(viral_load_table) - 1)) %dopar% 
{
  foreach ((2):(nrow(viral_load_table))) %dopar% 
  {
    if ((viral_load_table[i,"patient_id"] == viral_load_table[j,"patient_id"]) &
        (
          viral_load_table[i,"patient_visit_date"] == viral_load_table[j,"patient_visit_date"]
        ))
    {
     output[i] = "Plausible"
     output[j] = "Plausible"
...
kable(d, caption = "Table 1: Gender and Age by Health Facility Names of All Study Records", col.names = NA)
