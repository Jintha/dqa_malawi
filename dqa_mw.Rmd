...
numerators_all = c(number_of_all_complete_addresses,number_of_all_complete_phone_numbers,all_number_of_complete_weight_observations,number_of_all_complete_viral_loads,"0",number_of_all_patients_with_complete_adherence)

denominators_all = c(total_number_of_all_patients,total_number_of_all_patients,all_total_number_of_patient_visits,total_number_vl_all_patients,"0",number_of_all_patient_adherence_visits)

proportions_all = c(all_address_completeness_proportion,all_phone_number_proportion,proportion_all_complete_weight_documentation,proportion_of_all_complete_vl,"0",all_proportion_adherence)
...
proportion_of_all_palusible_addresses = number_of_all_plausible_addresses / number_of_all_complete_addresses * 100
...
for (i in 1:(nrow(viral_load_data_table_qech) - 1))
{
  for (j in (i + 1):(nrow(viral_load_data_table_qech)))
  {
    if ((viral_load_data_table_qech$patient_id[i] == viral_load_data_table_qech$patient_id[j]) &
        (
          viral_load_data_table_qech$patient_visit_date[i] == viral_load_data_table_qech$patient_visit_date[j]
        ))
    {
      viral_load_data_table_qech$vrl_day_plausible[i] = "Plausible"
      viral_load_data_table_qech$vrl_day_plausible[j] = "Plausible"
    }
  }
  print(i)
}
