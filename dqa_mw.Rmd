...
read_plus <- function(flnm) {
  print(flnm)
  read_csv(flnm) %>% mutate(health_facility_type = "Health Center") #%>% mutate(health_facility_type = ifelse(grep("dedza",flnm, ignore.case = TRUE),"District","Tertiary")) #mutate(health_facility_type = ifelse(grep("dedza",flnm >= 1) | grep("mangochi",flnm >= 1) | grep("nkhatabay",flnm >= 1),"District","Tertiary"))
}

# classify_hospital = function(patient_table){
#   mutate(patient_table, health_facility_type = ifelse(grep("dedza",patient_table$health_facility_type, ignore.case = TRUE),"District","Tertiary"))
# }
...
patient_address_classified = patient_address_df %>% lapply(classify_hospital(.))
