...
total_number_of_all_patients = all_health_facility_addresses %>% select(unique_patient_id) %>% unique.data.frame() %>% nrow()
...
all_total_number_of_patient_visits = all_health_facility_vitals %>% filter((encounter_datetime) > "2017-01-01") %>%  group_by(unique_patient_id,encounter_datetime) %>% unique.data.frame() %>% nrow()
...
all_number_of_complete_weight_observations = all_health_facility_vitals %>% filter(complete_weight == "Complete") %>% nrow()
...
all_number_of_height_observations = all_health_facility_vitals %>% filter(encounter_datetime >= "2017-01-01") %>% filter(height != "NULL") %>% group_by(unique_patient_id,health_facility_name) %>% slice(1) %>% nrow()
...
prescription_encounters = inner_join(all_health_facility_vitals,all_health_facility_drugs, by = "unique_patient_id") %>% group_by(all_health_facility_vitals$unique_patient_id,all_health_facility_vitals$health_facility_name) %>% slice(1)
...
update_names = function(drug_table){
   ...
   i = 1
   names_in_drug_column = drug_table$name 
   name_placed_in_quantity = drug_table$quantity
   while(i < nrow(drug_table)){
   test_evening = grepl("Evening",drug_table[i,]$name)
   if((test_evening == TRUE)){
       output[i] = drug_table[i,]$quantity 
       
  print("evening")
  }else{
       output[i] = drug_table[i,]$name
   print("madzulo")    
  }
  i = i+1
}
...
prescription_encounters_c = (update_names(prescription_encounters_b[101:200,]))
...
all_health_facility_adherence$health_facility_type = factor(all_health_facility_addresses$health_facility_type)
all_health_facility_adherence$health_facility_name = factor(all_health_facility_addresses$health_facility_name)
...
#   
#   by_month_vitals_documented = bmi_data_table %>%filter(weight > 0) %>%  group_by(yr = year(new_encounter_date),mn = month(new_encounter_date)) %>% dplyr::summarise(count_vitals = n())
#   
...
# 
#   combined_observations  = cbind(by_month_all_encounters,by_month_vitals_documented) %>% cbind(.,by_month_heights_documented) %>% select(yr,mn,count,count_heights,count_vitals)
#   
#   ggplot(combined_observations) + geom_line(aes(mn,count), size=1, color = "yellow") + geom_line(aes(mn,count_vitals), size = 1, color = "blue")
#   
#   
