...
patient_address_files = list.files(path="~/Box Sync/quantitative_data/patient_address", pattern="*.csv", full.names=TRUE)
patient_weight_files = list.files(path="~/Box Sync/quantitative_data/patient_weight_height", pattern="*.csv", full.names=TRUE)
patient_drugs_files = list.files(path="~/Box Sync/quantitative_data/patient_drugs_taken", pattern="*.csv", full.names=TRUE)
patient_drug_adherence_files = list.files(path="~/Box Sync/quantitative_data/patient_drug_adherence", pattern="*.csv", full.names=TRUE)
patient_vl_files = list.files(path="~/Box Sync/quantitative_data/hiv_viral_load", pattern="*.csv", full.names=TRUE)
...
read_plus <- function(flnm) {
  print(flnm)
  read_csv(flnm) %>% mutate(filename = flnm) #%>% mutate(health_facility_type = ifelse(grep("dedza",flnm, ignore.case = TRUE),"District","Tertiary")) #mutate(health_facility_type = ifelse(grep("dedza",flnm >= 1) | grep("mangochi",flnm >= 1) | grep("nkhatabay",flnm >= 1),"District","Tertiary"))
}

#Getting error here too
# classify_hospital = function(patient_table){
#   print(str(patient_table))
#   # for(i in 1:nrow(patient_table)){
#   # mutate(patient_table, health_facility_type = ifelse((grep("dedza",patient_table$filename, ignore.case = TRUE) >= 1),"District","Tertiary"))  
#   # } 
#  }

patient_address_df =
  patient_address_files %>% 
  map_df(~read_plus(.)) 
patient_address_df = mutate(patient_address_df, health_facility_type = "Health center")

#Getting Error here too
# for(i in 1:nrow(patient_address_df)){
#    mutate(patient_address_df[i,], health_facility_type = ifelse((grep("dedza",patient_address_df[i,]$filename, ignore.case = TRUE) >= 1),"District","Tertiary"))  
# } 

#patient_address_classified = patient_address_df %>% (classify_hospital(.))
...
adherence_data_table_qech     = read.csv("patient_drug_adherence/mzuzu_central_adherence.csv", stringsAsFactors = FALSE)
 ...
 #Build Table 1
 #Total number of unique patient records per health facility
 #Breakdown by age groups in increments of 5
 #Breakdown gender
 #Total number of patient visits that are being analyzed
 #Total number of encounters or observations being analyzed
 #
 ...
 table1(~gender + age|complete_address, data = id_data_table_qech)
