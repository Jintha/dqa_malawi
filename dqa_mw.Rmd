...
patient_drugs_files_district = list.files(path="~/Box Sync/quantitative_data/patient_drugs_taken/Health Center/", pattern="*.csv", full.names=TRUE)
...
#transform(health_facility_type = ifelse(grep("dedza",flnm, ignore.case = TRUE) >= 1,"District","Tertiary")) 
...
#Build tables by health facility type
...
#combine data frames
l = list(patient_address_df_health_center,patient_address_df_tertiary,patient_address_df_districts)
all_health_facility_addresses = rbindlist(l, use.names=TRUE)
