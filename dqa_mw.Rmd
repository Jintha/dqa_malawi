...
### Starting data quality assessment using the Kahn harmonized data quality framework with the data quality attributes of:
...
* Plausibility: This is the believability or truthfulness of observed data values, for example a patient with two viral load observations of 5000 and 700 on the same day will be implausible. We have 3 types of viral load that we are analyzing namely Unique plausibility, Temporal plausibility and Atemporal plausibility.
...
patient_vitals_healthcenter_mzuzu_df$encounter_datetime = as.Date(patient_vitals_healthcenter_mzuzu_df$encounter_datetime, "%m/%d/%y")  

patient_vitals_healthcenter_mzuzu_df$encounter_datetime = as.character(patient_vitals_healthcenter_mzuzu_df$encounter_datetime)


#Read patient drugs data frames
...
#Read patient viral loads dataframes
...
all_vl_patients_enocunters = all_health_facility_addresses %>% filter(health_facility_name != "Mapale" & health_facility_name != "MCH" )%>% unique.data.frame()
total_number_of_all_patients_vl = nrow(all_vl_patients_enocunters)
...
#all_patient_study_heights = join all study heights then select those study patients that have heights. So every patient check a height record. Return only the first record of the height.
#


all_height_encounters = all_health_facility_vitals %>% filter(encounter_datetime <= "2017-01-01") %>% filter(height != "NULL") %>% group_by(unique_patient_id) %>% slice(1)


patients_with_heights = left_join(all_study_patients,all_height_encounters, by = "unique_patient_id") %>% group_by(unique_patient_id) %>% slice(1)


patients_with_heights$pat_heights = ifelse(patients_with_heights$height == "NULL","Incomplete","Complete")
...
total_number_of_height_visits = patients_with_heights %>% filter(pat_heights =="Complete") %>% nrow()

proportion_height_encounters = total_number_of_height_visits / nrow(all_study_patients) *100
...
prescription_encounters = prescription_encounter_observations %>% filter(start_date_new == encounter_datetime) %>% group_by(unique_patient_id,encounter_datetime) %>% slice(1)
...
#We want to see how viral loads are collected at each facility. We can left join or use an iteration. I will use left join
...
#Read new dataset for health facilities. For LH-KCH, ZCH, MCH, and QECH


new_viral_load_dataset = fread("new_viral_load_data.csv")
new_viral_load_dataset = mutate(new_viral_load_dataset, health_facility_type = "Tertiary")
new_viral_load_dataset$startdate = as.Date(new_viral_load_dataset$startdate,"%d-%b-%y" )
new_viral_load_dataset$bleeddate = as.Date(new_viral_load_dataset$bleeddate,"%d-%b-%y" )
new_viral_load_dataset = new_viral_load_dataset %>% filter(health_facility_name != "MPC") %>% filter(bleeddate <="2018-12-31")


# since the viral loads are only for 3 health facilities, all of them being central hospitals
#Mzuzu central hospital not available
 
all_study_patients_selected = all_study_patients %>% filter(all_study_patients$health_facility_type == "Tertiary" & all_study_patients$health_facility_name !="MCH")


pats_with_vl = left_join(all_study_patients,patient_vl_encounters, by = "unique_patient_id") %>%  group_by(unique_patient_id) %>% slice(1)
 # label the fields with vl as patients with vl, otherwise label them as no result

pats_with_vl$recorded = "VL Result Received"
pats_with_vl$recorded = ifelse(pats_with_vl$patient_VL_results == "NULL","Waiting for result","VL Result Received")

bbvl = pats_with_vl %>% select(unique_patient_id,recorded,health_facility_name,patient_VL_results)
#Replace the NAs with a Not requested value
bbvl$recorded[is.na(bbvl$recorded)] <- 'Not requested'

#remove the value modifiers in the viral load results
bbvl$patient_VL_results = str_replace(bbvl$patient_VL_results, "=","")
bbvl$patient_VL_results = str_replace(bbvl$patient_VL_results, "<","")
bbvl$patient_VL_results = str_replace(bbvl$patient_VL_results, ">","")

#change the healthfacility_name and the recorded into factors. After changing them, then plot the factors into bargraphs and fill them with the different types of VL. That way you will see which health facilities actually had their VL filled and which ones did not.
#Point is to validate the viral load observations

bbvl$health_facility_name = as.factor(bbvl$health_facility_name)
bbvl$recorded = as.factor(bbvl$recorded)
bbvl$patient_VL_results = as.numeric(bbvl$patient_VL_results)


#
number_of_vl_visits = nrow(patient_vl_encounters)

proportion_of_vl_encounters = number_of_vl_visits / total_number_of_all_patients_vl * 100
...
#Calculate completeness for VL for new dataset of LH,QECH and ZCH

#viral patients 

tertiary_study_patients_test =  left_join(all_study_patients_selected,all_health_facility_vitals, by = "unique_patient_id") %>%  filter(encounter_datetime >= "2017-01-01") %>% group_by(unique_patient_id) %>% slice(1) 
...
number_of_tertiary_vl = new_viral_load_dataset %>% filter(bleeddate >= "2017-01-01") %>%  group_by(unique_patient_id) %>% slice(1) %>% nrow()

tertiary_study_patients =  left_join(all_study_patients_selected,all_health_facility_vitals, by = "unique_patient_id") %>%  filter(encounter_datetime >= "2017-01-01") %>% group_by(unique_patient_id) %>% slice(1) %>% nrow()

proportion_of_central_hospital_vl = number_of_tertiary_vl/tertiary_study_patients * 100
...
importance_priorities_b = importance_priorities

names(importance_priorities_b) = c("time_completed","vl_plausibility_vs_completeness","vl_completeness_vs_plausibility","vl_plausibility_vs_conformance","vl_conformance_vs_plausibility","vl_conformance_completeness","vl_completeness_vs_conformance","pid_plausibility_vs_completeness","pid_completeness_vs_plausibility","pid_plausibility_vs_conformance","pid_conformance_vs_plausibility","pid_conformance_vs_completeness","pid_completeness_vs_conformance","rete_plausibility_vs_completeness","rete_completeness_vs_plausibility","rete_plausibility_vs_conformance","rete_conformance_vs_plausibility","rete_conformance_vs_completeness","rete_completeness_vs_conformance") 

# Get the linkert responses only
importance_priorities_c = importance_priorities_b[,2:19]

factoredData = importance_priorities_c %>% mutate_if(is.character,funs(factor(.)))
bb_new = gather(factoredData,"Question","Ranking")
...
numerators_all = c(number_of_all_complete_addresses,
                   number_of_all_complete_phone_numbers,
                   all_number_of_complete_weight_observations,
                   number_of_all_complete_viral_loads,
                   number_of_all_complete_prescriptions,
                   number_of_all_patients_with_complete_adherence)

denominators_all = c(total_number_of_all_patients,
                     total_number_of_all_patients,
                     total_number_of_patient_visits,
                     total_number_vl_all_patients,
                     total_number_of_all_prescriptions,
                     number_of_all_patient_adherence_visits)

proportions_all = c(all_address_completeness_proportion,
                    all_phone_number_proportion,
                    proportion_all_complete_weight_documentation,
                    proportion_of_all_complete_vl,
                    all_prescription_completeness_proportion,
                    all_proportion_adherence)
...
variable_names_encounters = c("Current Patient Address",
                              "Patient Mobile Phone Number",
                              "Heights of Patient",
                              "Weights of Patient",
                              "Prescriptions Made",
                              "Drug Adherence",
                              "HIV Viral Load","Tertiary hospital VL")


encounter_numerators  = c(number_of_all_complete_addresses,
                          number_of_all_complete_phone_numbers,
                          total_number_of_height_visits,
                          total_number_of_weight_visits,
                          number_of_prescription_encounters,
                          number_of_adherence_visits,
                          number_of_vl_visits,
                          number_of_tertiary_vl)

encounter_denominators = c(total_number_of_all_patients,
                           total_number_of_all_patients,
                           total_number_of_all_patients,
                           total_number_of_patient_visits,
                           total_number_of_patient_visits,
                           total_number_of_patient_visits,
                           total_number_of_all_patients_vl,
                           tertiary_study_patients)

proportion_encounters = c(all_address_completeness_proportion,
                          all_phone_number_proportion,
                          proportion_height_encounters,
                          proportion_weight_encounters,
                          proportion_drug_encounters,
                          proportion_adherence_encounters,
                          proportion_of_vl_encounters,
                          proportion_of_central_hospital_vl) 
...
#age group classification. Put in increments of 10
#agebreaks <- c(15,20,25,30,35,40,45,50,55,60,65,70,75,80,85,500)
#agelabels <- c("15-19","20-24","25-29","30-34",
#               "35-39","40-44","45-49","50-54","55-59","60-64","65-69",
#               "70-74","75-79","80-84","85+")


agebreaks <- c(15,25,35,45,55,500)
agelabels <- c("15-24","25-34","35-44","45-54","55+")
...
setDT(tertiary_study_patients_test)[ , agegroups := cut(age.x, 
                                breaks = agebreaks, 
                                right = FALSE, 
                                labels = agelabels)]

viral_load_table_one = table1(~gender+agegroups+age.x|health_facility_name.x, data = tertiary_study_patients_test, output = "pandoc")


#Calculate the Interquartile range which is a calculation of the difference between the upper quartile range and the lower quartile range
hc_iqr_dt = all_health_facility_addresses %>% filter(health_facility_type == "Health Center")
dh_iqr_dt = all_health_facility_addresses %>% filter(health_facility_type == "District")
ch_iqr_dt = all_health_facility_addresses %>% filter(health_facility_type == "Tertiary")


#Calculate the Interquartile range which is a calculation of the difference between the upper quartile range and the lower quartile range for viral load patients
kch_iqr_dt = tertiary_study_patients_test %>% filter(health_facility_name.x == "KCH")
qech_iqr_dt = tertiary_study_patients_test %>% filter(health_facility_name.x == "QECH")
zch_iqr_dt = tertiary_study_patients_test %>% filter(health_facility_name.x == "ZCH")
...
#Temporal address plausibility

# require to have the date that the patient started ARVs to compute that temporal plausibility. May take it from the dispensations. Join patient addresses and the patient addresses and select the first prescription day. take that as the first dispensation date.
#get the first dispensed drugs. verify that it's ARVs only.
#
first_drugs = all_health_facility_drugs  %>% filter(name %in% list_of_arv_drugs) %>% group_by(name,start_date_new,unique_patient_id) %>% slice(1)

addresses_with_first_art_date = left_join(all_health_facility_addresses,first_drugs, by="unique_patient_id") %>% group_by(unique_patient_id,visit_date) %>% slice(1)

#calculate intervals between visit date and art_start_date. we want the visit date of address collection to be on the same date of starting ART or after. We do not want a date that is 7 days earlier than starting ART

addresses_with_first_art_date$visit_date  = as.Date(addresses_with_first_art_date$visit_date, "%Y-%m-%d")

addresses_with_first_art_date$time_days = interval(
  addresses_with_first_art_date$visit_date,
  addresses_with_first_art_date$start_date_new
) / days(1)

#Flag all addresses that were collected before the patient started taking ARVs

addresses_with_first_art_date$temporal_address = ifelse(addresses_with_first_art_date$time_days < 0 ,"Not Plausible","Plausible")

number_temporal_palusible_addresses = addresses_with_first_art_date %>% filter(temporal_address == "Plausible") %>% nrow()

proportion_of_temporal_plausible_addresses = number_temporal_palusible_addresses /total_number_of_all_patients  * 100

#

#Unique address plausibility
#flag all duplicate entries. So far returns none.
all_health_facility_addresses %>% group_by(unique_patient_id,district,ta,village) %>% 
  dplyr::mutate(duplicate_addresses = n()>1) 

#Return all the unique entries
unique_addresses = all_health_facility_addresses %>% select(unique_patient_id,district,ta,village) 
duplicate_entries = unique_addresses[duplicated(unique_addresses),]
...
#Atemporal plausibility - checking the range of possible values
...
#Temporal plausibility will be all phone numbers registered before the ART start date. In this case the ART start date is the day of the first recorded ART dispensation for a patient at that clinic. We shall take all_patient_addresses and join with the prescriptions to check the first ART dispensation. will be the same as the physical address because they are collected at the same time.


proportion_temporal_plausible_cellnumbers = proportion_of_temporal_plausible_addresses



#Unique plausibility. Will screen to check if patient has more than 1 mobile phone number listed. Each patient should have 1 active phone number that they may be reached on. Same phone number assigned to multiple patients?? we can have the scenario of patients sharing phone numbers. But if a patient has more than 1 mobile number. That could point to a problem.

# Flag all duplicate mobile numbers. Then count the duplicate numbers that will be flagged as True
all_health_facility_addresses %>% group_by(unique_patient_id,cell_number) %>% 
  dplyr::mutate(duplicate_mobiles = n()>1) 

#Return only the duplicate numbers and later count them
unique_mobiles = all_health_facility_addresses %>% select(unique_patient_id,cell_number) 
duplicate_mobiles = unique_mobiles[duplicated(unique_mobiles),]
...
#Atemporal plausibility checking weight or height range values
#
...
#Atemporal plausibility checking weight values with weight for age
#Separate weight checking from height checking
...
#check weight values whose creation date is different from the visit date. Do we have weight taken before a patient started taking ARVs?

study_period_temporal_vitals = left_join(study_period_vitals,first_drugs, by = "unique_patient_id") %>% 
group_by(unique_patient_id,encounter_datetime)  %>% slice(1)

study_period_temporal_vitals$encounter_datetime = as.Date(study_period_temporal_vitals$encounter_datetime, "%Y-%m-%d")
#flag any vitals observations made before the patient ART start date. 

study_period_temporal_vitals$time_days = interval(
  study_period_temporal_vitals$start_date_new,
  study_period_temporal_vitals$encounter_datetime
) / days(1)

study_period_temporal_vitals = study_period_temporal_vitals %>% filter(weight_new >0)
study_period_temporal_vitals$temporal_weight = ifelse((study_period_temporal_vitals$time_days < 0), "Not plausible","Plausible")
study_period_temporal_vitals = study_period_temporal_vitals %>% filter(height_new >0)
study_period_temporal_vitals$temporal_height = ifelse((study_period_temporal_vitals$time_days < 0), "Not plausible","Plausible")
# 

number_temporal_plausibility_weight = study_period_temporal_vitals %>%  filter(temporal_weight == "Not plausible") %>% nrow()
number_temporal_plausibility_height = study_period_temporal_vitals %>%  filter(temporal_height == "Not plausible") %>% nrow()

proportion_temporal_weght  = number_temporal_plausibility_weight / total_number_of_weight_visits * 100




#unique plausibility. check patients that have more than one weight observation on the same date.
#unique_weight_visits = study_period_vitals %>% 
#flag all duplicate weight observations

unique_weights = study_period_vitals %>% select(unique_patient_id,encounter_datetime,weight_new) %>% group_by(unique_patient_id,encounter_datetime,weight_new) %>% 
  dplyr::mutate(duplicate_weights = n()>1)
number_of_patients_with_unique_weights = unique_weights %>% filter(duplicate_weights == FALSE) %>% nrow()

proportion_all_unique_plausible_weights = number_of_patients_with_unique_weights / total_number_of_weight_visits * 100


# Plausible Height Values. Get all the plausible height observations. Group by unique patient and then count only those with plausible heights.

patients_with_heights = mutate(patients_with_heights, study_age_in_months = patients_with_heights$age.x*12)  
patients_with_heights$gender = ifelse(patients_with_heights$gender == "F" | patients_with_heights$gender == "Female", "Female","Male")

new_pat_height_vector = character(nrow(patients_with_heights))
new_pat_height_vector = patients_with_heights$height
new_plausible_vitals_height = character(nrow(patients_with_heights))

if(find.file("patients_with_heights.csv") == "patients_with_heights.csv"){
  patients_with_heights = fread("patients_with_heights.csv")
}else{
  
for(i in 1:nrow(patients_with_heights)){
if(!is.na(patients_with_heights[i,]$pat_heights)){
#check age first, then look up values
if(patients_with_heights[i,]$study_age_in_months <= 215){
height_study_age_in_months = patients_with_heights[i,]$study_age_in_months
new_study_sex = ifelse(patients_with_heights[i,]$gender == "Male",0,1)
validating_record = weight_height_lookup_table %>% filter(age_in_months == height_study_age_in_months & sex == new_study_sex)
#
      print("ndalowa!")
      print(new_pat_height_vector[i])
      new_plausible_vitals_height[i] = ifelse(as.numeric(new_pat_height_vector[i]) >= validating_record$low_height & as.numeric(new_pat_height_vector[i]) <= validating_record$high_height,"Plausible","Not Plausible" )
#}
}else
  {# check absolute weight and height for older patients
    #check absolute height first
    new_plausible_vitals_height[i] = ifelse(as.numeric(new_pat_height_vector[i]) >= 10 & as.numeric(new_pat_height_vector[i])<= 200,"Plausible","Not Plausible")
  }
}else
{
  print("No height collected")
}
}
patients_with_heights$plausible_height = new_plausible_vitals_height  
write.csv(patients_with_heights, file = "patients_with_heights.csv")  
}


number_of_all_plausible_height_observations = patients_with_heights %>% filter(plausible_height == "Plausible") %>% nrow()

proportion_all_plausible_heights = number_of_all_plausible_height_observations/total_number_of_height_visits * 100

# unique height plausibility

unique_heights = study_period_vitals %>% select(unique_patient_id,encounter_datetime,height_new) %>% group_by(unique_patient_id,encounter_datetime,height_new) %>% dplyr::mutate(duplicate_heights = n()>1)
number_of_all_plausible_unique_height_observations = unique_heights %>% filter(duplicate_heights == FALSE) %>% nrow()

# Temporal height proportions

proportion_temporal_height = number_temporal_plausibility_height / total_number_of_height_visits * 100
...
#Atemporal plausibility searching for arv dispensations within expected quantities of an average of 3 month dispensations
...
#Unique plausibility for prescription
#every patient should have one and only one prescription of the same drug on the same day in line with the dosage and quantity of the ingredients
unique_prescriptions = arv_prescription_observations %>% select(unique_patient_id,start_date_new,name,quantity) %>% group_by(unique_patient_id,start_date_new,name) %>% dplyr::mutate(duplicate_prescriptions = n()>1)
number_of_unique_prescriptions = unique_prescriptions %>% filter(duplicate_prescriptions == FALSE) %>% nrow()


#Temporal prescripions


#Plausible Adherence should (Atemporal plausibility checking the range of possible plausibility)
#
all_health_facility_adherence$adherence_in_percentage = as.numeric(all_health_facility_adherence$adherence_in_percentage)


all_health_facility_adherence$plausible_adherence = ifelse(all_health_facility_adherence$adherence_in_percentage >= 0 & all_health_facility_adherence$adherence_in_percentage <= 105, "Plausible","Not plausible")
...
#Temporal adherence plausibility

#

#unique adherence plausibility
#count patients with one and only one adherence observation for a particular drug on a particular day

unique_adherence_counts = all_health_facility_adherence %>% filter(start_date_new >= "2017-01-01") %>%  select(unique_patient_id,start_date_new,ARV_regimen_name) %>% group_by(unique_patient_id,start_date_new,ARV_regimen_name) %>% dplyr::mutate(duplicate_heights = n()>1)
number_of_unique_adherence = unique_adherence_counts %>% filter(duplicate_heights == FALSE) %>% nrow()
...
#Atemporal plausibility - range of dates
...
all_health_facility_vl$patient_visit_date  = as.Date(all_health_facility_vl$patient_visit_date, "%Y-%m-%d")
...
#Tertiary hospitals
new_viral_load_dataset_at = new_viral_load_dataset

new_viral_load_dataset_at$time_months = interval(
  new_viral_load_dataset_at$startdate,
  new_viral_load_dataset_at$bleeddate
) %/% months(1)


new_viral_load_dataset_at  = new_viral_load_dataset_at %>% group_by(unique_patient_id) %>% slice(1)


#order by bleeddate asc group by patient_id and bleeddate slice 1.
#already in ascending order for bleeddate so I select only the first record for each patient
#
#Atemporal plausibility there is a range in the expected date of starting ARVs
all_months_until_first_vl = all_health_facility_vl %>% group_by(patient_id, ART_start_date) %>% slice(1)
all_months_until_first_vl$plausible = ifelse((all_months_until_first_vl$time_months >= 5 & all_months_until_first_vl$time_months <= 36), "Plausible","Not Plausible")

new_viral_load_dataset_at$plausible = ifelse((new_viral_load_dataset_at$time_months >= 5 & new_viral_load_dataset_at$time_months <= 36), "Plausible","Not Plausible")



vl_atemporal_plausibility = all_months_until_first_vl %>%  filter(plausible == "Plausible") %>% nrow()

new_vl_atemporal_plausibility = new_viral_load_dataset_at %>%  filter(plausible == "Plausible") %>% nrow()
...
#   all_health_facility_vl_duplicates = fread("viral_load_table.csv")
# }else{
#   all_health_facility_vl_duplicates = plausible_viral_loads_day_results(all_health_facility_vl)  
# }

#vl_unique_plausibility = all_health_facility_vl_duplicates %>% filter(vrl_day_plausible == "Plausible") %>% nrow
vl_unique_plausibility = 0
...
vl_earlier_than_censor_date$plausible_firstdate =  ifelse(all_health_facility_vl$patient_visit_date <= "2012-01-01","Not plausible","Plausible")

# Check if there are some tests that were conducted before the time that VL testing started in Malawi
new_viral_load_dataset_at$plausible_firstdate =  ifelse(new_viral_load_dataset_at$bleeddate < "2012-01-01","Not plausible","Plausible")

# Check if there are tests that have been conducted before a patients started taking ARVs
# 
new_viral_load_dataset_at$plausible_bleeddate =  ifelse(new_viral_load_dataset_at$bleeddate < new_viral_load_dataset_at$startdate,"Not plausible","Plausible")

new_temporal_plausibility_censor = new_viral_load_dataset_at %>% filter(plausible_firstdate =="Plausible") %>% nrow()

new_temporal_plausibility_sta_date = new_viral_load_dataset_at %>% filter(plausible_bleeddate =="Plausible") %>% nrow()

new_viral_load_temporal_plausibility = (number_of_tertiary_vl - new_temporal_plausibility_sta_date) + new_temporal_plausibility_censor

# all_number_of_plausible_viral_loads_day_results = all_health_facility_vl %>% filter(plausible_viral_load == "Plausible") %>% nrow()
vl_temporal_plausibility = vl_earlier_than_censor_date %>% filter(plausible_firstdate == "Plausible") %>% group_by(unique_patient_id) %>% slice(1) %>% nrow()


number_of_all_plausible_viral_loads =  vl_temporal_plausibility + vl_atemporal_plausibility + vl_unique_plausibility


proportion_of_all_plausible_vl = number_of_all_plausible_viral_loads / nrow(all_health_facility_vl) * 100

#Temporal plausibility - patients with viral load date earlier than start date
...
#
variable_names_plausibility = c("Current Patient Physical Address","Patient Mobile Phone Number","Weights of Patient","Heights of Patient","Prescription Made","Drug Adherence","Unique HIV Viral Load","Atemporal HIV Viral Load","Temporal HIV Viral Load")
...
numerators_all_plausibility = c(number_of_all_plausible_addresses,
                                ...
                                vl_unique_plausibility,new_vl_atemporal_plausibility,
                                new_viral_load_temporal_plausibility)
 
denominators_all_plausibility = c(number_of_all_complete_addresses,
                                  number_of_all_complete_phone_numbers,
                                  all_number_of_complete_weight_observations,
                                  total_number_of_height_visits,
                                  number_of_arv_prescription_observations,
                                  number_of_all_patient_adherence_visits,
                                  number_of_tertiary_vl,
                                  number_of_tertiary_vl,
                                  number_of_tertiary_vl)

proportions_all_plausibility = c(proportion_of_all_plausible_addresses,
                                 proportion_of_all_plausible_numbers,
                                 proportion_all_plausible_weights,
                                 proportion_all_plausible_heights,
                                 all_prescription_plausibility_proportion,
                                 proportion_all_plausible_adherence,
                                 (vl_unique_plausibility/number_of_tertiary_vl*100),
                                 (new_vl_atemporal_plausibility/number_of_tertiary_vl*100),
                                 (new_viral_load_temporal_plausibility/number_of_tertiary_vl*100))
...
kable(viral_load_table_one, caption = "Table 1: Gender and Age by Health Facility Name of all Viral Load Records", col.names = NA)

#Put the Interquartile Range for Age for different health facility types

IQR(ch_iqr_dt$age)
IQR(hc_iqr_dt$age)
IQR(dh_iqr_dt$age)
IQR(tertiary_study_patients_test$age.x)
IQR(kch_iqr_dt$age.x)
IQR(qech_iqr_dt$age.x)
IQR(zch_iqr_dt$age.x)
...
kable(number_of_patient_visits_per_health_facility, caption = "Number of patient visits by health facility name and health facility type (Jan 2017 to Dec 2018)")
...
ggsave("~/Dropbox/dissertation_data_analysis/addresscompleteness.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)



ggplot(cd, aes(address_section, fill = address_value)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
    labs(title = "Completeness of Patient Address Components Documentation",  y = "Percentage of Completeness", fill="Documentation Status", colour = "Documentation Status", x = "Address Components") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(),panel.background = element_blank(), legend.title = element_blank()) +
facet_grid(.~health_facility_type) 
  
ggsave("~/Dropbox/dissertation_data_analysis/addresscompleteness2.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)


bc = cd %>% group_by(health_facility_type,address_section,address_value) %>% 
 dplyr::summarize(count=n())

#write.csv((bc), file = "bunched_addresses.csv")

new_cd = fread("bunched_addresses.csv")
new_cd = new_cd %>% mutate_if(is.character,funs(factor(.)))

ggplot(new_cd, aes(x = address_section,y=percentage,  fill = address_value)) + 
    geom_bar(stat="identity", position="dodge") + geom_text(aes(label = percentage, vjust = -0.5)) +  labs(title = "Completeness of Patient Address Components Documentation",  y = "Percentage of Completeness", fill="Documentation Status", colour = "Documentation Status", x = "Address Components") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(),panel.background = element_blank(), legend.title = element_blank()) +
facet_grid(.~health_facility_type) 
   
ggsave("~/Dropbox/dissertation_data_analysis/addresscompleteness3.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)

ggplot(new_cd, aes(x = address_section,y=percentage,  fill = address_value)) + 
    geom_bar(stat="identity", position="fill") + geom_text(aes(label = percentage, vjust = -0.5)) +  labs(title = "Completeness of Patient Address Components Documentation",  y = "Percentage of Completeness", fill="Documentation Status", colour = "Documentation Status", x = "Address Components") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(),panel.background = element_blank(), legend.title = element_blank()) +
facet_grid(.~health_facility_type) 

ggsave("~/Dropbox/dissertation_data_analysis/addresscompleteness5.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)
...
ggsave("~/Dropbox/dissertation_data_analysis/addresscompleteness4.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)

new_cd<- ddply(new_cd, .(address_section),
                     transform, pos = cumsum(percentage) - (0.5 * percentage))

p4 <- ggplot() + geom_bar(aes(y = percentage, x = address_section, fill = address_value), data = new_cd,
                           stat="identity") +
  geom_text(data=new_cd, aes(x = address_section, y = pos,
                                  label = paste0(percentage,"%")), size=4) +
  theme(legend.position="bottom", legend.direction="horizontal", legend.title = element_blank()) +
facet_grid(.~health_facility_type) 
p4

#+ geom_hline(yintercept = total_number_of_all_patients, color = "yellow", size = 2 )
...
ggsave("~/Dropbox/dissertation_data_analysis/arvsboxplot.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)
...
ggsave("~/Dropbox/dissertation_data_analysis/weightdistribution.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)
...
ggsave("~/Dropbox/dissertation_data_analysis/heightdistribution.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)


ggplot(bbvl, aes(health_facility_name)) + geom_bar(stat="count",aes(fill=recorded)) + geom_text(stat = 'count',aes(label =..count.., vjust = -0.2, fill=recorded)) +
  labs(title = "2017 to 2018 Health Facility Patient Viral Load Observations", y = "Count", x = "Health Facility Name") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(),panel.background = element_blank(), legend.title = element_blank(), axis.text.x = element_text( angle=45))


ggsave("~/Dropbox/dissertation_data_analysis/viral_load_observations.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)


ggplot(bbvl, aes(health_facility_name, patient_VL_results)) + geom_boxplot() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(),panel.background = element_blank(), legend.title = element_blank()) +
  labs(title = "2017 to 2018 Health Facility Patient Viral Load Averages", y = "Count", x = "Health Facility Name")

ggsave("~/Dropbox/dissertation_data_analysis/viral_load_boxplot.png", plot = last_plot(), device = "png", path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)


bb_new$Ranking <- factor(bb_new$Ranking, levels = c("Very important", "Important", "Moderately Important", "Slightly Important", "Not Important")) 

ggplot(bb_new) + geom_bar(mapping = aes(x=Question,fill=Ranking)) + scale_x_discrete(breaks=c("vl_plausibility_vs_completeness","vl_completeness_vs_plausibility","vl_plausibility_vs_conformance","vl_conformance_vs_plausibility","vl_conformance_completeness","vl_completeness_vs_conformance","pid_plausibility_vs_completeness","pid_completeness_vs_plausibility","pid_plausibility_vs_conformance","pid_conformance_vs_plausibility","pid_conformance_vs_completeness","pid_completeness_vs_conformance","rete_plausibility_vs_completeness","rete_completeness_vs_plausibility","rete_plausibility_vs_conformance","rete_conformance_vs_plausibility","rete_conformance_vs_completeness","rete_completeness_vs_conformance") ,
                      labels=c("Is plausibility of higher priority than completeness for Viral load monitoring?", "Is completeness of higher priority than plausibility for Viral load monitoring?", "Is plausibility of higher priority than conformance for Viral load monitoring?","Is conformance of higher priority than plausibility for Viral load monitoring?","Is conformance of higher priority than completeness for Viral load monitoring","Is completeness of higher priority than conformance for Viral load monitoring","Is plausibility of higher priority than completeness for patient identification?", "Is completeness of higher priority than plausibility for patient identification?", "Is plausibility of higher priority than conformance for patient identification?","Is conformance of higher priority than plausibility for patient identification?","Is conformance of higher priority than completeness for patient identification","Is completeness of higher priority than conformance for patient identification","Is plausibility of higher priority than completeness for patient retention?", "Is completeness of higher priority than plausibility for patient retention?", "Is plausibility of higher priority than conformance for patient retention?","Is conformance of higher priority than plausibility for patient retention?","Is conformance of higher priority than completeness for patient retention","Is completeness of higher priority than conformance for patient retention")) +    coord_flip() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank(),panel.background = element_blank(), legend.title = element_blank()) +
labs(title = "Stakeholder Data Quality \nCharacteristics Priorities", x = "Data Quality Characteristic Priority Question", y = "Count of Data Quality Characteristic Importance Ranking") + scale_fill_discrete(name="Importance \nRanking",
                         breaks=c("Very important", "Important", "Moderately Important","Slightly Important","Not Important"),
                         labels=c("Very important", "Important", "Moderately Important","Slightly Important","Not important"))


ggsave("~/Dropbox/dissertation_data_analysis/importance_priorities.png", plot = last_plot(), device = "png", path = NULL,
