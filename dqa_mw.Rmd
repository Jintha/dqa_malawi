...
study_patient_visits = all_health_facility_vitals %>% filter((encounter_datetime) > "2017-01-01") %>%  group_by(unique_patient_id,encounter_datetime) %>% unique.data.frame() 
...
all_number_of_complete_weight_observations = all_health_facility_vitals %>% filter(encounter_datetime >= "2017-01-01") %>%  filter(complete_weight == "Complete") %>% nrow()

all_weight_encounters = all_health_facility_vitals %>% filter(encounter_datetime >= "2017-01-01") %>% filter(weight != "NULL") %>% group_by(unique_patient_id,health_facility_name) %>% slice(1) 
...
all_height_encounters = all_health_facility_vitals %>% filter(encounter_datetime >= "2017-01-01") %>% filter(height != "NULL") %>% group_by(unique_patient_id,health_facility_name) %>% slice(1) 

total_number_of_height_visits = nrow(all_height_encounters)

proportion_height_encounters = total_number_of_height_visits / total_number_of_all_patients *100
...
#Create Completeness Table for Patient Visits or Encounters
...
encounter_denominators = c(total_number_of_all_patients,total_number_of_all_patients,total_number_of_patient_visits,total_number_of_all_patients,total_number_of_patient_visits,total_number_of_patient_visits,total_number_of_all_patients)
...
ggplot(study_period_vitals, aes(health_facility_name,height_new)) + geom_boxplot()
...
for(i in 1:nrow(study_period_vitals)){
  pat_id = study_period_vitals[i,]$unique_patient_id
  vitals = all_study_patients %>% filter(unique_patient_id == pat_id ) 
  if(vitals$age !is.null() == TRUE){
   study_period_vitals_age[i] = vitals$age
   study_period_vitals_sex[i] = vitals$gender
  }
}
...
study_period_vitals = mutate(study_age_in_months = study_period_vitals$age*12)
weight_height_lookup_table = fread("~/Box Sync/wtht_rangecheck.csv", col.names = c("age_in_months", "sex","low_height", "high_height", "low_weight", "high_weight"))

plausible_vitals_weight = character(nrow(study_period_vitals))
plausible_vitals_height = character(nrow(study_period_vitals))

for(i in 1:nrow(study_period_vitals)){
  validating_record = weight_height_lookup_table %>% filter(age_in_months == study_period_vitals[i,]$study_age_in_months & sex = study_period_vitals[i,]$sex)
  pat_weight = study_period_vitals[i,]$weight
  pat_height = study_period_vitals[i,]$height
  plausible_vitals_weight[i] = ifelse(pat_weight >= validating_record$low_weight & pat_weight <= validating_record$high_weight,"Plausible","Not Plausible" )
  if(pat_height !is.null){
    plausible_vitals_height[i] = ifelse(pat_weight >= validating_record$low_height & pat_weight <= validating_record$high_height,"Plausible","Not Plausible" )
  }
}
...
number_of_plausible_height_observations = 1
proportion_plausible_heights = number_of_plausible_height_observations/number_of_complete_height_observations * 100
number_of_all_plausible_height_observations = 1
proportion_all_plausible_heights = number_of_all_plausible_height_observations/all_number_of_complete_height_observations * 100
