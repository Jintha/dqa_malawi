...
all_weight_encounters = all_health_facility_vitals %>% filter(encounter_datetime >= "2017-01-01") %>% filter(weight != "NULL") %>% group_by(unique_patient_id,encounter_datetime) %>% slice(1) 
...
#all_health_facility_drugs = update_names(all_health_facility_drugs)
...
if(find.file("study_patient_vitals_plausible.csv") == "study_patient_vitals_plausible.csv"){
  study_patient_vitals = fread("study_patient_vitals_plausible.csv")
  ...
  {# check absolute weight and height for older patients
    ...
    plausible_vitals_height[i] = ifelse(pat_height_vector[i] >= 10 & pat_height_vector[i]<= 200,"Plausible","Not Plausible")
    plausible_vitals_weight[i] = ifelse(pat_weight_vector[i] >= 0 & pat_weight_vector[i]<= 250,"Plausible","Not Plausible")
  }
}
...
proportion_plausible_weights = number_of_plausible_weight_observations/total_number_of_weight_visits * 100
...
ggplot(all_health_facility_drugs)
...
all_patients_with_plausible_vl_timeframe = all_months_until_first_vl %>% filter(patient_visit_date >= "2017-01-01") %>%  filter(time_months == "Plausible") %>% nrow()
...
viral_load_visit_dates = character (nrow(viral_load_table))
viral_load_patient_ids = viral_load_table$unique_patient_id
viral_load_visit_dates = viral_load_table$patient_visit_date
foreach (1:(nrow(viral_load_table) - 1)) %dopar% 
{
  foreach ((2):(nrow(viral_load_table))) %dopar% 
  {
    if ((viral_load_patient_ids[i] == viral_load_patient_ids[j]) &
        (
          viral_load_table_visit_dates[i] == viral_load_table_visit_dates[j]
        ))
    {
     output[i] = "Plausible"
     output[j] = "Plausible"
...
viral_load_table$plausible_viral_load = output
...
number_of_all_plausible_viral_loads = all_number_of_plausible_viral_loads_day_results + all_patients_with_plausible_vl_timeframe + number_of_plausible_vl_earliest
